  ...
  return (
    <div className="w-full h-[90px]">
      ...
      <nav className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-6 py-4 bg-[#111]/70 backdrop-blur-md shadow-md ">
        ...
        {/* LOGO */}
        <div className="logo text-white pt-2 text-4xl lg:text-6xl font-bold ">
          <Link href="/">Maxnet</Link>
        </div>

        {/* Desktop Nav */}
        <div className="hidden lg:flex gap-10 text-white text-lg font-medium nav-components items-center">
          <Link href="/">Home</Link>
          ...
          <Link
            href="/shop"
            className="bg-gradient-to-r from-cyan-400 to-cyan-600  px-4 py-2 rounded-md hover:text-white"
          >
            SHOP
          </Link>
        </div>
      ...
      {/* Mobile Nav Menu */}
      {isOpen && (
        <div className="top-[90px] left-0 w-full bg-[#111]/70 backdrop-blur-md flex flex-col items-center gap-6 py-6 lg:hidden text-white z-40 nav-components mob-nav fixed transition-all">
